...
# Middleware order: Security headers then CORS
@app.middleware("http")
async def add_security_headers(request: Request, call_next):
    ...
    # Less restrictive CSP to avoid blocking dev tools and internal fetches
    response.headers["Content-Security-Policy"] = "default-src 'self' http://localhost:8003; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline';"
    return response
    ...
    try:
        ...
        if user:
            ...
            if user.subscription_status == "free":
                ...
                # 1. Monthly Reset (30 days)
                if user.token_balance <= 0:
                    if user.last_token_reset and (now - user.last_token_reset).total_seconds() > 30 * 24 * 3600:
                        user.token_balance = 100
                        user.last_token_reset = None
                        db.commit()
                    else:
                        from fastapi import HTTPException
                        next_reset = (user.last_token_reset or now) + datetime.timedelta(days=30)
                        reset_str = next_reset.strftime("%m/%d/%Y, %I:%M:%S %p")
                        raise HTTPException(status_code=403, detail=f"Monthly token limit reached|{reset_str}")
...
async def payment_callback(request: Request, db: Session = Depends(get_db)):
    ...
    if result.get('status') == 'success':
        ...
        if user:
            ...
            user.token_balance = 100 # Reset to 100 for premium
            user.last_token_reset = None
            db.commit()
