...
async def student_node(state: GraphState):
    ...
    if not isinstance(combined, CombinedPermitResult):
        ...
        combined = CombinedPermitResult(
            ...
            agencies=["G├╢├º ─░daresi", "University Student Affairs"],
    ...
    for i, st in enumerate(combined.steps):
        details.append(StepDetail(
            id=i + 1,
            title=st.title,
            responsible="Agent" if "e-Devlet" in st.description or "Agent" in st.description or "e-─░kamet" in st.description else "Human/Agent",
            status="pending",
