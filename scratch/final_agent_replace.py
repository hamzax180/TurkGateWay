...
def replace_in_file(file_path):
    ...
    # Fix the "agent" mistake
    content = content.replace('Agent', 'Agent')
    content = content.replace('agent', 'agent')
    content = content.replace('AGENT', 'AGENT')
    
    # Ensure any remaining Agent is gone
    content = content.replace('Agent', 'Agent')
    content = content.replace('agent', 'agent')
    content = content.replace('AGENT', 'AGENT')
...
for root, dirs, files in os.walk(root_dir):
    # CRITICAL: Exclude system/library folders
    if any(x in root for x in ['venv', 'node_modules', '.next', '.git']):
        continue
    for file in files:
        if file.endswith(extensions):
            replace_in_file(os.path.join(root, file))

print("Final comprehensive replacement done!")
